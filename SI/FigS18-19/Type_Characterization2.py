...
def Type_Characterization_test(results,a=1000,b=2000):
    Vt=results[:,0]#virus time sequence
    IL6=results[:,26]
    if Vt[-1]<1e-6 and max(IL6)<a:
        return 1
    if np.max(Vt)>1 and Vt[-1]<1e-6 and a<=max(IL6)<=b:
        return 2
    if np.max(Vt)>1 and Vt[-1]<1e-6 and max(IL6)>b:
        return 3
    if High(Vt,1) and max(IL6)>b:
        return 4
    return 0