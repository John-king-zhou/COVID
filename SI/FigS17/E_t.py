...
for i in range(1,5,1):
    try:
        mean_e = np.loadtxt('rmean%i_e.txt'%i)
        mean_logR = np.loadtxt('rmean%i_R.txt'%i)
    except:
        mean_e=[]
        data=np.loadtxt('rSpl_Para%i.txt'%i)
        e_list=[]
        e1_list=[]
        e2_list=[]
        e12_list=[]
        ...
        np.savetxt('rmean%i_e.txt'%i,mean_e)
        np.savetxt('rmean%i_R.txt'%i,mean_logR)
...
time=np.arange(0,28.1,dt)
for mode in [1,2,3,4]:
    mean_e=np.loadtxt('rmean%i_e.txt'%mode)
    mean_logR=np.loadtxt('rmean%i_R.txt'%mode)
    # t=time[indices]
    # print(mode,indices)
...
fig.savefig('rE-t-geomean.png')
fig.savefig('rE-t-geomean.svg')
plt.show()