...
Para1=np.loadtxt('rSpl_Para1.txt')
Para2=np.loadtxt('rSpl_Para2.txt')
Para3=np.loadtxt('rSpl_Para3.txt')
Para4=np.loadtxt('rSpl_Para4.txt')
...
for i in [0,1,2,3]:
    print(i)
    Typei=Paras[i]
    Traj=[[] for j in range(37)]
    try:
        Aver_Results=np.loadtxt('rMean%i.txt'%(i+1))
        Std_Results=np.loadtxt('rStd%i.txt'%(i+1))
    except:
        Aver_Results=[]
        Std_Results=[]
        ...
        np.savetxt('rMean%i.txt'%(i+1),Aver_Results)
        np.savetxt('rStd%i.txt'%(i+1),Std_Results)
...
fig.savefig('Illustration_random.svg')
fig.savefig('Illustration_random.png',dpi=300)
plt.show()