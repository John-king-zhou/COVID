...
def get_results(Para,v0,CD8Tm,Bm,A):
    time0=np.arange(0, 50, 0.1)
    initial = [v0, 0, Para[52] / Para[62], 0, 0, Para[53] / Para[65], 0, 0, Para[159]/2, 0, 0, 0, 0, 0, 0,
...
if __name__=='__main__':
    ...
    logv_seq = np.arange(-3, 0, 0.2)
    Nv = len(logv_seq)
    v_seq = 10 ** logv_seq
    try:
        Protection=np.loadtxt('Protection_v0.txt')
    except:
        sz=1000
        #Memory distribution
        mean = [-1, -1]
        std = [1, 1]
        Protection=np.zeros((Nv,8))
        for mode in range(1,5,1):
            ...
            for j in range(Nv):
                ...
                args=[]
                rand_inds=np.random.choice(sz)
                for i in range(sz):
                    args.append([Parameter,v_seq[j],random_points_T[i],random_points_Ab[i],1])
                count=0
                Imm=0
                Mild=0
                print('logv0=%1.1f'%logv_seq[j],'time used: %1.1f s'%(timer.perf_counter()-t0))
        ...
        np.savetxt('Protection_v0.txt',Protection)
    pool.close()
    fig,ax=plt.subplots(1,1,figsize=(3.5,2))
    for mode in range(1,5,1):
        ax.plot(logv_seq, Protection[:,mode-1]*100, c=ggcolors[mode], linewidth=1.5, ls='-')
        ax.plot(logv_seq, Protection[:,mode+3]*100, c=ggcolors[mode], linewidth=1.5, ls='--')
    ...
    ax.set_xlabel('log$_{10}$ initial inoculum',labelpad=2)
    ...
    fig.savefig('efficacy_v0.png')
    fig.savefig('efficacy_v0.svg')
    plt.show()