    ...
    for Ab,i in zip(Ab_seq,range(len(Ab_seq))):
        ...
        if i==0:
            ...
            for j in range(100):
                sol = get_time_seq(Para, v0, Tm_tmp[-1], CD4Tm, Bm, Ab, A)
                max_I.append(max(sol[:,26]))
                if  (max_I[-1]-IL6_thr)*(max_I[-2]-IL6_thr)<=0:
                    break
                if max_I[-1]<=IL6_thr:
                    Tm_tmp.append(Tm_tmp[-1]-dTm)
                elif max_I[-1]>IL6_thr:
                    Tm_tmp.append(Tm_tmp[-1]+dTm)
            dTm=0+dTm_min
...
if __name__=='__main__':
    ...
    # for full protection
    Bmseq=np.linspace(0,0.08,20)
    # CD8Tmseq=np.linspace(0,1,20)
    Abseq=np.linspace(0,250,20)

    try:
        dt = np.load(path+'\\3D_AbBmTm_severePrevent_m%d.npy' %mode, allow_pickle=True).item()
        ...
        np.save(path+'\\3D_AbBmTm_severePrevent_m%d.npy' %mode, dt)
        pool.close()
    ...
    ax3d.plot_surface(XX, YY, ZZ_Tm, alpha=0.3, color='green')
    ...
    ax3d.set_title('Severe Prevention Surface')
