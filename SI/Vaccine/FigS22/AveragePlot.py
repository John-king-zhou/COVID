...
def averPlot(axes,time,Mean,color,mode):
    Vars=['log$_{10}$(nCoV)', 'If', 'H', 'D', 'APC$^l$', 'APC$^u$', 'NK', 'Neut', 'CD4$^+$T$_N$', 'CD4$^+$T$_A$',
          ...
          'CTL', 'CD8$^+$T$_M$', 'GC B', 'Plasma B',  r'$B_M$' ,'IL-2', 'IL-4', 'IL-6', r'IL-10/TGF-$\beta$',
          r'TNF-$\alpha$', r'IFN-$\gamma$', 'Ab', 'A','lg(R)', r'$\epsilon$', r'$\epsilon_i$', r'$\epsilon_a$',
          r'$\epsilon_k$', r'$\epsilon_c$', r'$log_{10}\left(J_{S,CD4}\right)$', r'$log_{10}\left(J_{S,CD8}\right)$']
    ...
    for i in range(Mean.shape[1]):
        ...
        x=Mean[:,i]
        if mode==4:
            ax.plot(time, x, linewidth=2, c=color, label='Mode 1')
        elif mode in [1,2,3]:
            ax.plot(time, x, linewidth=2, c=color, label='Mode %i'%(mode+1))
        elif mode in [4.1, 4.2]:
            ax.plot(time, x, linewidth=2, c=color, label='Mode %s'%(mode))
        else:
...
def averPlot_Supply(axes,time,Mean,color,mode,std):
    Vars=['log$_{10}$(nCoV)', 'If', 'H', 'D', 'APC$^l$', 'APC$^u$', 'NK', 'Neut', r'$CD4+T_N$', r'$CD4+T_A$',
          'Th1', 'Th2', 'Th17', 'Tfh', 'Treg$^a$', 'Treg$^r$', r'$CD4+T_M$', r'$CD8+T_N$', r'$CD8+T_A$',
          'CTL', r'$CD8+T_M$', 'GC B', 'Plasma B', r'$B_M$' ,'IL-2', 'IL-4', 'IL-6', r'IL-10/TGF-$\beta$',
          r'TNF-$\alpha$', r'IFN-$\gamma$', 'Ab', 'A',r'$log_{10}(R_t)$', r'$\epsilon$', r'$\epsilon_i$', r'$\epsilon_a$',
          r'$\epsilon_k$', r'$\epsilon_c$', r'$log_{10}\left(J_{S,CD4}\right)$', r'$log_{10}\left(J_{S,CD8}\right)$']
    axes[0].plot(time,np.zeros(len(time)),c='k',lw=1)
    for i in range(Mean.shape[1]):
        ax=axes[i]
        ...
        if mode==4:
            ...
            ax.plot(time, x, linewidth=2, c=color, label='Mode %s'%(mode))
            ax.fill_between(time,x+std_y,x-std_y,color=color,alpha=0.2)
        else:
            ax.plot(time, x, linewidth=2, c=color, label='Mode %i'%(mode))
        ax.xaxis.set_major_locator(xmajorLocator)
        ax.set_title(Vars[i],fontsize=11,pad=3)
